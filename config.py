...
from  datasets.pedes_icfg import IcfgPedes
from datasets.pedes_rstp import RSTPPedes
...
def data_config(dataset, image_dir, anno_dir, batch_size, split, max_length, transform, vocab_path='', min_word_count=0, cap_transform=None):
    ...
    if dataset == 'CUHK-PEDES':
        ...
        data_split = IcfgPedes(image_dir, anno_dir, split, max_length, transform,
                               vocab_path=vocab_path, min_word_count=min_word_count, cap_transform=cap_transform)
    elif dataset == 'RSTPReid':
        data_split = RSTPPedes(image_dir, anno_dir, split, max_length, transform,
                               vocab_path=vocab_path, min_word_count=min_word_count, cap_transform=cap_transform)
...
def get_image_unique_cuhk(image_dir, anno_dir, batch_size, split, max_length, transform):
    return CuhkPedes(image_dir, anno_dir, split, max_length, transform).unique

def get_image_unique_icfg(image_dir, anno_dir, batch_size, split, max_length, transform):
    return IcfgPedes(image_dir, anno_dir, split, max_length, transform).unique

def get_image_unique_rstp(image_dir, anno_dir, batch_size, split, max_length, transform):
    return RSTPPedes(image_dir, anno_dir, split, max_length, transform).unique
...
def log_config(args, ca):
    ...
    filename = args.checkpoint_dir +'/' +current_time +'_'+ca+ '.txt'
