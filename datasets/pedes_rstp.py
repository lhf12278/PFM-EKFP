...
import datasets.preprocess_rstp as preprocess_rstp

class Namespace:
    def __init__(self, **kwargs):
        self.__dict__.update(kwargs)

class RSTPPedes(data.Dataset):
    '''
    Args:
    ...
    def add_caption_to_data(self, split_data, data):
        ...
        fname_dict = {jj['img_path']:jj for jj in split_data}
        judge = {jj['img_path']: 0 for jj in split_data}
    ...
    def get_data_from_json(self):
        ...
        if self.vocab_path == '':
            vocab = preprocess_rstp.build_vocab(split_data, args, write=False)
        else:
            vocab = self.load_vocab(self.vocab_path)

       
        split_metadata = preprocess_rstp.process_metadata(self.split, split_data, args, write=False)
        split_decodedata = preprocess_rstp.process_decodedata(split_metadata, vocab)
        data = preprocess_rstp.process_dataset(self.split, split_decodedata, args, write=False)
    ...
    def load_vocab(self, vocab_path):    
        ...
        vocab = preprocess_rstp.Vocabulary(word_to_idx, len(word_to_idx))
        return vocab   
