...
import datasets.preprocess_icfg as preprocess_icfg

class Namespace:
    def __init__(self, **kwargs):
        self.__dict__.update(kwargs)

class IcfgPedes(data.Dataset):
    '''
    Args:
    ...
    def get_data_from_json(self):
        ...
        if self.vocab_path == '':
            vocab = preprocess_icfg.build_vocab(split_data, args, write=False)
        else:
            vocab = self.load_vocab(self.vocab_path)

       
        split_metadata = preprocess_icfg.process_metadata(self.split, split_data, args, write=False)
        split_decodedata = preprocess_icfg.process_decodedata(split_metadata, vocab)
        data = preprocess_icfg.process_dataset(self.split, split_decodedata, args, write=False)
        
        return data

    def load_split(self, split):
        # split_root = '/media/ysd/1305D8BDB8D46DEE/ysd/data/ICFG-PEDES/ICFG-PEDES.json'
    ...
    def load_vocab(self, vocab_path):    
        ...
        vocab = preprocess_icfg.Vocabulary(word_to_idx, len(word_to_idx))
        return vocab   
