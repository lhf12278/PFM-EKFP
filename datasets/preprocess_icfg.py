...
ImageMetaData = namedtuple('ImageMetaData', ['id', 'image_path', 'captions', 'caption_tokens', 'split'])
ImageDecodeData = namedtuple('ImageDecodeData', ['id', 'image_path', 'captions', 'captions_id', 'split'])
...
def process_metadata(split, data, args, write=True):
    ...
    for img in data:
        ...
        id = img['id']
        caption_tokens = img['processed_tokens']
        captions = img['captions']

        # print(captions)

        id_to_captions.setdefault(id, [])
        id_to_captions[id].append(caption_tokens)
        # assert split == img['split'], 'error: wrong split'
        image_metadata.append(ImageMetaData(id, filepath, captions, caption_tokens, split))
...
def process_decodedata(data, vocab):
    ...
    for img in data:
        ...
        for cap in img.caption_tokens:
            cap_to_vec.append([vocab.word_to_id(word) for word in cap])
        image_decodedata.append(ImageDecodeData(img.id, image_path, captions, cap_to_vec, img.split))
...
def create_dataset_sort(split, data):
    ...
    for img in data:
        label = img.id
        image = [ImageDecodeData(img.id, img.image_path, img.captions, [caption_id], img.split) for caption_id in img.captions_id]
        if label in images:
...
def write_dataset(split, data, args, label_range=None, write=True):
    ...
    for img in data:
        ...
        captions.append(img.captions[0])
        labels.append(img.id)
        images_path.append(img.image_path)

    #N = len(images)
    data = {'caption_id': caption_id, 'captions': captions, 'labels':labels, 'images_path':images_path}
...
def parse_args():
    ...
    parser.add_argument('--img_root', type=str, default='./data/ICFG-PEDES/imgs')
    parser.add_argument('--json_root', type=str, default='./data/ICFG-PEDES/ICFG-PEDES.json')
    parser.add_argument('--out_root', type=str, default='./data/ICFG-PEDES/processed_data')
